...
- `--port` 是必填的统一外部端口，范围 1–65535。被控端在该数值上同时绑定 TCP 与 UDP：TCP 负责认证、会话控制、剪贴板和鼠标形状；所有鼠标移动、按钮、滚轮及键盘事件都以单个数据报发往固定 UDP `--port`。输入包携带当前会话随机令牌、单调序号和事件类型；错误会话及乱序旧包直接丢弃，不等待 ACK，也不重复发送。视频、诊断和反馈继续使用会话协商的 UDP 端口。
- 经 FRP 等端口转发时，同一个外部端口分别配置一条 TCP 和一条 UDP 映射即可，不再转发随机输入端口。单个 UDP 按键释放包可能丢失；关闭键鼠转发或断开输入控制 TCP 时仍执行全量释放，避免跨会话残留按键。
...
- 推荐无需管理员权限的目录：`%LOCALAPPDATA%\Programs\FRD\v1.pre12\`，不同版本分目录保留。
- 本机发布包为 `D:\pub\FRD\v1.pre12.zip` 与同名 SHA-256 文件；工作区也保留 `artifacts/v1.pre12/` 构建副本。推送 `v*` 标签会由 GitHub Actions 在 Windows runner 上重新构建，并创建带 ZIP 和校验文件的 GitHub Pre-release；标签必须与项目版本一致。
...
- TCP 保留认证、会话控制、剪贴板、鼠标形状和输入生命周期。FRP 只需为同一个端口配置一条 TCP 与一条 UDP 映射，不再需要随机键鼠 UDP 端口。
...
进一步阅读：[使用与配置](docs/使用.md)、[目标与验收](docs/目标与验收.md)、[发布说明](docs/v1.pre12-发布说明.md)、[静态检查](docs/v1.pre12-静态检查.md)、[第三方许可证](THIRD-PARTY-NOTICES.md)。
