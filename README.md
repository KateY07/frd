...
- `--port` 是必填的唯一外部端口，范围 1–65535。被控端在该数值上同时绑定 TCP 与 UDP：TCP 负责认证、会话控制、剪贴板和鼠标形状；UDP 在同一个 socket 内按包类型复用视频、反馈、诊断、鼠标、滚轮和键盘。控制握手不再交换随机视频、诊断或发送端口。
- 主控先向固定 UDP `--port` 发送带会话随机值的注册包，被控端只向该已认证来源回送视频和诊断。注册最多重试 5 次；未收到确认时主控明确报告 UDP `--port` 不可达，不进入静默无画面/无输入状态。输入包携带当前会话随机值、单调序号和事件类型；错误会话及乱序旧包直接丢弃，不等待 ACK，也不重复发送。
- 经 FRP、Ardui/ardJ 等端口转发时，同一个外部端口只需各配置一条 TCP 和 UDP 映射，所有 FRD 主被控通信均经过该端口。主控 socket 的临时 UDP 源端口只是操作系统和转发器建立回程映射所需的客户端来源，不是额外监听、配置或转发端口。单个 UDP 按键释放包可能丢失；关闭键鼠转发或断开输入控制 TCP 时仍执行全量释放，避免跨会话残留按键。
- ArdUi 开发组已确认同一映射允许多个并发、独立的 TCP 连接。FRD 当前每个会话最多使用 4 条，低于转发入口的 8 条上限；FRD 最大 UDP 数据报为 1172 字节，低于转发层的 1482 字节上限。
...
- 正常退出优先关闭进程主窗口并等待结束，保证捕获、网络及输入状态得到清理。源码中通过 SYSTEM 辅助进程显示锁屏与 UAC 安全桌面，并在被控端授权键鼠后向该进程转发输入；双机 Demo 的 UAC、锁屏画面及键鼠操作已人工确认。辅助进程尚未纳入便携包，不能将本次开发测试当作 v1.pre11 的无人值守能力。被控端连接时在右下角显示受控提示，可断开连接或撤销剪贴板、键鼠权限；窗口从普通桌面捕获中排除。声音尚未实现。
...
- 推荐无需管理员权限的目录：`%LOCALAPPDATA%\Programs\FRD\v1.pre13\`，不同版本分目录保留。
- 本机发布包为 `D:\pub\FRD\v1.pre13.zip` 与同名 SHA-256 文件；工作区也保留 `artifacts/v1.pre13/` 构建副本。推送 `v*` 标签会由 GitHub Actions 在 Windows runner 上重新构建，并创建带 ZIP 和校验文件的 GitHub Pre-release；标签必须与项目版本一致。
...
- 修复 v1.pre12 只把键鼠收敛到固定 UDP `--port`、视频/反馈/诊断仍动态协商端口的协议回归。远程会话现在只暴露同一个 TCP/UDP `--port`，UDP 注册、视频、反馈、诊断及全部输入由单 socket 分流。
- 删除控制握手中的 `VideoPort`、`DiagnosticPort` 和 `SenderPort`。主控通过带会话随机值的 UDP 注册建立回程地址；被控端确认注册后才启动编码发送。UDP 映射缺失会在约 2.5 秒内明确失败，不再静默丢失画面或键鼠。
- 本机单端口转发器已覆盖视频双向流量、诊断、反馈和输入，并通过 100 ms 至 10 s 同会话停顿恢复回归。

## v1.pre12 更新

- 被控端在同一个 `--port` 同时监听 TCP 与 UDP。所有鼠标移动、按钮、滚轮和键盘事件统一以单个、无 ACK 的 UDP 数据报发往此固定端口；输入包校验会话随机令牌与递增序号，错误或旧包直接丢弃。
- TCP 保留认证、会话控制、剪贴板、鼠标形状和输入生命周期。该版本只取消了随机键鼠端口；视频、反馈和诊断仍动态协商，因此其“FRP 只需一组固定映射”的说明不完整，已由 v1.pre13 修正。
- 固定端口 UDP 输入、伪造/旧包拒绝、辅助 TCP 暂停期间的输入以及 100 ms 至 10 s 会话停顿恢复已在本机回归。

## v1.pre11 更新

- 自动发布改用 GitHub Release REST API，并更新官方 Actions 到 Node.js 24 版本；云端错误会直接保留 HTTP 诊断。
...
进一步阅读：[使用与配置](docs/使用.md)、[目标与验收](docs/目标与验收.md)、[发布说明](docs/v1.pre13-发布说明.md)、[静态检查](docs/v1.pre13-静态检查.md)、[第三方许可证](THIRD-PARTY-NOTICES.md)。
