...
- 推荐无需管理员权限的目录：`%LOCALAPPDATA%\Programs\FRD\v1.pre11\`，不同版本分目录保留。
- 发布脚本的输出目录为 `artifacts/v1.pre11/`，包含便携 ZIP、README 和 SHA-256。推送 `v*` 标签会由 GitHub Actions 在 Windows runner 上重新构建，并创建带 ZIP 和校验文件的 GitHub Pre-release；标签必须与项目版本一致。
...
进一步阅读：[使用与配置](docs/使用.md)、[目标与验收](docs/目标与验收.md)、[发布说明](docs/v1.pre11-发布说明.md)、[静态检查](docs/v1.pre11-静态检查.md)、[第三方许可证](THIRD-PARTY-NOTICES.md)。
