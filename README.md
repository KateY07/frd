...
- 推荐无需管理员权限的目录：`%LOCALAPPDATA%\Programs\FRD\v2.pre1\`，不同版本分目录保留。
- 本机发布包为 `D:\pub\FRD\v2.pre1.zip` 与同名 SHA-256 文件；工作区也保留 `artifacts/v2.pre1/` 构建副本。推送 `v*` 标签会由 GitHub Actions 在 Windows runner 上重新构建，并创建带 ZIP 和校验文件的 GitHub Pre-release；标签必须与项目版本一致。
...
- 固定 UDP 接收循环完整接收后再校验包长，超长包直接丢弃；瞬态网络错误保留 socket，真正不可恢复的错误立即通知会话，避免输入/反馈后台静默停收。
- 输入执行移至独立线程，安全桌面本机 ACK 等待不再阻塞公共收包。待执行事件最多 128 个，只合并连续鼠标移动；饱和时丢弃待执行积压并先释放按键/按钮。禁用、断开时清理队列并等待在途执行结束。
- 诊断回调失败时记录错误并停用该回调，视频和输入继续；UDP 注册五次超时后明确提示检查固定 UDP 端口转发。
- 回归要求恢复后的新解码帧、被控端输入执行、诊断和反馈计数增长；补充原 TCP 连接上的双向剪贴板及鼠标形状恢复验证。
- 保持 v1.pre13 的单端口约定及 1172 字节 UDP 包长。本机协议回归不替代真实 ArdUi/ARD 公网验收。
...
进一步阅读：[使用与配置](docs/使用.md)、[目标与验收](docs/目标与验收.md)、[发布说明](docs/v2.pre1-发布说明.md)、[静态检查](docs/v2.pre1-静态检查.md)、[第三方许可证](THIRD-PARTY-NOTICES.md)。
